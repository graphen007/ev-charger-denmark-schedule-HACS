...
function isoToSlot(record) {
    return { start: record.HourDK, value: record.SpotPriceDKK / 1000 };
}
async function fetchJson(url) {
    const res = await fetch(url);
    if (!res.ok)
        throw new Error(`Energinet API ${res.status}: ${url}`);
    return res.json();
}
function fmtDate(d) {
...
async function fetchSpotPrices(area, startDate, endDate) {
...
async function fetchPrices(area) {
    ...
    const [todaySlots, tomorrowSlots] = await Promise.all([
        fetchSpotPrices(area, todayStr, todayStr).catch(() => []),
        fetchSpotPrices(area, tomorrowStr, tomorrowStr).catch(() => []),
    ]);
...
async function fetchForecast(area) {
    ...
    let windCapacityPct = null;
    try {
        const tomorrowStr = fmtDate(tomorrow);
        const windData = await fetchJson(`${BASE}/Forecasts_5Min?start=${tomorrowStr}T00:00&end=${tomorrowStr}T23:59&limit=288`);
    ...
    try {
        const tomorrowStr = fmtDate(tomorrow);
        const co2Data = await fetchJson(`${BASE}/CO2Emis?start=${tomorrowStr}T00:00&end=${tomorrowStr}T23:59&limit=24`);
